...
const navItems = [
  ...
  { href: "/wallet", label: "資產", icon: (active: boolean) => (
    <svg className="w-6 h-6" fill={active ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24">
...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-lg border-t border-gray-100 z-50 pb-safe">
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className="relative flex flex-col items-center flex-1 py-2">
              <div className={`${isActive ? "text-orange-500" : "text-gray-400"} transition-colors`}>
                {item.icon(isActive)}
              </div>
              {!isCenter && (
                <span className={`text-[10px] font-black mt-1 ${isActive ? "text-black" : "text-gray-400"}`}>
                  {item.label}
                </span>
              )}
            </Link>
