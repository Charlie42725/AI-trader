...
export default function AnalysisDetailPage() {
  ...
  if (loading && !job) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen">
        <div className="w-12 h-12 border-3 border-[var(--border)] border-t-[var(--primary)] rounded-full animate-spin" />
  ...
  if (error && !job) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen px-4">
        <div className="w-16 h-16 rounded-full bg-[var(--danger)]/20 flex items-center justify-center mb-4">
  ...
  return (
    <div className="min-h-screen pb-8">
      {/* 頂部導航 */}
      <header className="sticky top-0 z-10 bg-[var(--bg-app)] border-b border-[var(--border-light)]">
        <div className="flex items-center gap-4 px-4 py-3">
          <button
            onClick={() => router.back()}
            className="w-10 h-10 rounded-full bg-[var(--bg-surface)] flex items-center justify-center"
          >
          ...
          <div className="flex-1">
            <h1 className="font-bold text-lg">{job.ticker}</h1>
            <p className="text-xs text-[var(--text-muted)]">{job.date}</p>
          </div>
      ...
      {(job.status === "pending" || job.status === "running") && (
        <div className="px-4 py-12">
          <div className="flex flex-col items-center text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">AI 正在分析中</h3>
        ...
        <div className="px-4 py-6">
          <div className="card p-4">
            <div className="flex items-start gap-3">
              <div className="w-10 h-10 rounded-lg bg-[var(--danger)]/20 flex items-center justify-center flex-shrink-0">
                <svg className="w-5 h-5 text-[var(--danger)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-medium text-[var(--danger)] mb-1">分析失敗</p>
                <p className="text-sm text-[var(--text-secondary)] break-words">{job.error}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {job.status === "completed" && r && (
        <div className="px-4 py-6 space-y-6">
          ...
          <section>
            <h2 className="font-semibold mb-3 flex items-center gap-2">
              <span className="w-1.5 h-4 bg-[var(--warning)] rounded-full" />
              交易團隊
            </h2>
          ...
          <section>
            <h2 className="font-semibold mb-3 flex items-center gap-2">
              <span className="w-1.5 h-4 bg-[var(--danger)] rounded-full" />
              風險管理
            </h2>
            <div className="space-y-2">
